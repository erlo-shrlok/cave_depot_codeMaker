...
def generate_js_functions(all_sql_statements):
    ...
    for comment, table, content in all_matches:
        ...
        filtered_columnscommon = [col for col in columns if
                                  col not in ["id", "create_user", "delete_time", "is_delete", "create_time",
                                              "user_cave"]]
        ...
        all_values = [special_columns[col] if col in special_columns else f"${{{to_camel_case(col)}}}" for col in
                      all_columns]
        result = ", ".join(f"{col}='{val}'" for col, val in zip(all_columns, all_values))

        js_function = f"// {comment}\n"
        js_function += f"""export const update{to_camel_case(table.capitalize())}SQL = function(queryParameters) {{
    return new Promise((resolve, reject) => {{
        ...
        plus.sqlite.executeSql({{
            ...
            fail: function(e) {{
                console.log('数据更新失败：' + JSON.stringify(e));
                reject(e); 
            }}
        }});
    }});
